...
class KiraProcessor(
        ...
        resolver.getSymbolsWithAnnotation(kiraAnnClass.canonicalName).forEach { function ->
            ...
            if (parameterSuppliers.any { it is ParameterSupplier.Empty }) {
                environment.logger.warn(
                    message = Errors.suitableProviderNotFound(parameterSuppliers, fullFunName),
                    symbol = function
                )
                return@forEach
            }

            parameterSuppliers as List<ParameterSupplier.Provided>
    ...
    ) = TypeSpec.classBuilder(className = ClassName(generatedPkgName, "Kira_$fileName"))
